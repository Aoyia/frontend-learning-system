...
B. 在主线程空闲时采用多线程 Worker 强行同步虚拟 DOM 树：
   React 的并发渲染不是 DOM 多线程，Commit 阶段仍然要在主线程提交 DOM 变更。
C. 通过在编译期剔除所有不参与交互的无状态 React 组件以缩减运行时开销：
   昂贵过滤、排序、图表计算仍然存在，必要时要用 memo、Worker、分页或虚拟列表处理。
D. 限制所有的 HTTP 请求使其在服务端合并后再发往客户端：
...
B. 直接给所有组件加 memo
C. 将页面内的所有交互状态都强制转化为无状态组件以避开 Hook 追踪
D. 强行阻断所有异步网络请求的加载以保证主线程能优先响应交互
答案：A
解析：
...
- 逐项拆解：
  ...
  - C：将页面内的所有交互状态都强制转化为无状态组件以避开 Hook 追踪不选。它把 React 性能问题简化成单一 API，忽略了 Render/Commit、状态边界或实际瓶颈取证。
  - D：强行阻断所有异步网络请求的加载以保证主线程能优先响应交互不选。它把 React 性能问题简化成单一 API，忽略了 Render/Commit、状态边界或实际瓶颈取证。
- 面试表达：React 性能题要先定位卡顿来源，再区分调度、组件边界、数据规模和 DOM 提交成本。
...
A. 大列表筛选结果更新：
   ...
   ```
B. 客户端输入框中当前键入字符的物理视觉呈现：
   不适合放进 transition；用户输入本身是高优先级反馈，延迟会直接造成“不跟手”。
C. 确认支付时提交按钮的即时禁用置灰状态：
