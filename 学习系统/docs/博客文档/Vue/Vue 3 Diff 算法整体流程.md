...
### Q1 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，Vue 3 diff 性能提升的关键之一是编译阶段引入了什么标记机制？
A. VNodeHash
B. PatchFlag
C. TreeShakingFlag
D. HydrateMask
答案：B
解析：PatchFlag 让运行时可以快速定位"哪些节点/属性可能变化"。

### Q2 [multiple]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，以下哪些属于 Vue 3 编译时优化手段？
A. 静态提升（hoist）
B. 事件监听器缓存
C. Block tree 动态子节点收集
D. 每次渲染都重建全部 vnode
答案：ABC
解析：前三项是 Vue 3 的核心优化；"每次全量重建"恰好是要避免的。

### Q3 [judgment]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，在更新阶段，Block Tree 的目标是让 diff 复杂度更接近"动态节点数量"而非"整棵模板大小"。
答案：对
解析：正确。dynamicChildren 是只包含动态节点的扁平入口。

### Q4 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，在 `patchKeyedChildren` 中，先做"前置扫描 + 后置扫描"的主要目的是什么？
A. 为了生成 source map
B. 快速跳过头尾稳定区，缩小中间复杂 diff 范围
C. 为了兼容 IE11
D. 为了让 key 失效
答案：B
解析：头尾相同节点可直接复用，复杂度集中在中间不稳定区域。

### Q5 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，`newIndexToOldIndexMap` 中值为 `0` 通常表示什么？
A. 该节点在旧序列中存在且无需移动
B. 该节点是静态节点
C. 该新节点在旧序列中不存在，需要新建
D. 该节点需要立即卸载
答案：C
解析：0 是保留值，表示"新节点在旧列表找不到映射"。

### Q6 [multiple]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，在中间乱序区使用 LIS（最长递增子序列）优化时，以下哪些说法正确？
A. 位于 LIS 的节点可保持相对顺序，通常不需要移动
B. 不在 LIS 的老节点更可能触发移动
C. LIS 的主要价值是减少 DOM move 次数
D. 使用 LIS 后就不需要处理新增和删除
答案：ABC
解析：LIS 只优化"移动"成本，新增与删除仍需单独处理。
...
D. 为了跳过 key 校验
答案：B
解析：DOM 插入语义类似 `insertBefore(newNode, anchor)`。倒序处理时，当前节点右边的新节点通常已经被挂载或移动到正确位置，因此可以直接作为当前节点的 anchor，减少插入位置判断复杂度。

### Q8 [judgment]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，首次挂载阶段，静态节点也会被创建；只是在后续更新中通常会被优化跳过。
答案：对
解析：正确。首次 mount 需要真实创建 DOM；优化收益主要体现在后续 patch。

### Q9 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，在 block 优化路径中，`dynamicChildren` 更接近哪种角色？
A. 完整节点树的镜像副本
B. 只包含动态节点的扁平"快速通道"
C. 服务端 hydration 的 HTML 字符串
D. 编译期产物映射表
答案：B
解析：它不是完整树，而是用于更新阶段快速定位动态节点的扁平集合。

### Q10 [multiple]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，哪些情况更可能导致节点进入"移动"分支而不是"仅更新"分支？
A. key 存在且新索引出现逆序
B. 新旧节点类型相同且位置不变
C. 中段乱序导致 maxNewIndexSoFar 回退
D. 节点在 LIS 中
答案：AC
解析：逆序和索引回退会让 moved=true；LIS 内节点通常可保持不动。

### Q11 [judgment]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，没有 key 的列表在复杂重排场景下，通常比有 key 的列表更容易做稳定复用。
答案：错
解析：错误。缺少 key 会降低匹配稳定性，重排场景更容易出现不必要 patch。

### Q12 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，为什么 v-for 相关 fragment 常被单独视为一个 block 边界？
A. 因为它只包含静态节点
B. 因为其子节点数量/顺序更可能变化，需要独立处理
C. 因为 v-for 不参与 diff
D. 因为必须关闭 key
答案：B
解析：列表结构变化频繁，编译器会在 block 设计上做边界划分。

### Q13 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，在中段 diff 中，如果某旧节点在 `keyToNewIndexMap` 中找不到对应新索引，应该如何处理？
A. 移动到末尾
B. 卸载该旧节点
C. 强制复用到最近节点
D. 标记为静态并跳过
答案：B
解析：找不到映射说明该节点已不存在于新序列，应执行卸载。

### Q14 [multiple]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，面试里描述 Vue3 diff 优势时，哪些说法更准确？
A. 编译时标记 + 运行时定向 patch
B. 静态提升可减少重复 vnode 创建与 patch 成本
C. LIS 优化目标是减少 DOM move 次数
D. Vue3 完全不需要 patch children 逻辑
答案：ABC
解析：最后一项明显不成立，很多路径仍需要 children diff。

### Q15 [judgment]
在 Vue 3 列表 diff 的最后移动/插入阶段，如果运行时从后向前遍历新 children，当前节点要插入时通常可以把它右侧已经处理好的 DOM 节点作为 `anchor`，也就是“插到这个右侧节点前面”，这有助于稳定插入逻辑。
答案：对
解析：判断为对。倒序遍历时，右侧节点先被处理，位置更确定；当前节点可以用右侧节点作为 `insertBefore` 的参照，避免前向遍历时右侧节点尚未稳定导致 anchor 难以确定。

### Q16 [single]
在 Vue 3 渲染器的列表 Diff 与编译优化场景中，当用户手写 render 且缺乏编译器注入优化标记时，更常见的结果是？
A. 自动补齐全部 PatchFlag 并保持同等优化
B. 部分优化路径收益下降，更依赖通用 diff 逻辑
C. 直接无法渲染
D. 自动退化到 Vue2 运行时
答案：B
解析：编译器信息不足会让优化程度下降，但不会影响正确性。
