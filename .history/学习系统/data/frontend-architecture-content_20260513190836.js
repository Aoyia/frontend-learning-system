...
这个类型让错误组合在调用方直接报错，比文档提醒更可靠。`,
      ...
      references: ['TypeScript Handbook', 'TypeScript satisfies Release Notes', 'TypeScript Project References 文档', 'Zod 官方文档'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'workspace', desc: '包管理器提供的多包依赖管理机制。' },
        { term: 'task pipeline', desc: '任务之间的依赖图，例如先 build 依赖包再 build 应用。' },
        { term: 'affected', desc: '根据变更文件和依赖图计算受影响包。' },
        { term: 'remote cache', desc: '跨机器复用构建结果，降低 CI 时间。' },
        { term: 'package boundary', desc: '包之间的 API 和依赖边界，防止随意跨层引用。' },
        { term: 'Changesets', desc: '多包版本变更和发布管理工具。' },
      ],
...
\`^build\` 表示先构建依赖包，再构建当前包。这是 Monorepo 构建正确性的基础。`,
      ...
      references: ['pnpm Workspace 文档', 'Turborepo Task 文档', 'Nx Affected 文档', 'Changesets 文档'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'host', desc: '主应用，负责路由、布局、资源加载和公共能力。' },
        { term: 'remote', desc: '子应用或远程模块，独立开发和发布。' },
        { term: 'sandbox', desc: '限制子应用全局变量、副作用和运行时污染。' },
        { term: 'style isolation', desc: '避免子应用样式互相覆盖。' },
        { term: 'shared dependency', desc: '多个应用共享框架或库，需治理版本兼容。' },
        { term: 'communication contract', desc: '跨应用通信协议，最好少而稳定。' },
      ],
...
跨应用通信要像 API 一样设计契约，不要把整个 store 暴露给所有子应用。`,
      ...
      references: ['webpack Module Federation 文档', 'single-spa 官方文档', 'qiankun 官方文档', 'MDN CustomEvent 文档'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'API contract', desc: '消费方依赖的稳定行为集合。' },
        { term: 'design token', desc: '颜色、间距、圆角、字号等设计变量。' },
        { term: 'ARIA', desc: '可访问性语义，组件库公开行为的重要组成。' },
        { term: 'visual regression', desc: '对比 UI 截图，发现样式和布局意外变化。' },
        { term: 'codemod', desc: '自动迁移代码的脚本，用于大规模 breaking changes。' },
        { term: 'deprecation', desc: '弃用策略，在删除前提供提示和迁移周期。' },
      ],
