...
3. 进一步拓展大厂面试追问：如果项目中有些第三方库（如旧版 jQuery）频繁直接对 innerHTML 赋字符串，开启 Trusted Types 导致大面积报错，如何平滑过渡？可以配置一个“Default Policy”（默认策略），当向 Sink 传入字符串时，浏览器会自动调用该默认策略进行过滤，以向后兼容不受控的代码库。`),
        multiple('在现代 Web 安全体系中，配置内容安全策略（CSP）主要能够帮助防御和限制哪些行为？', ['允许执行的合法脚本、样式及媒体资源的域名来源白名单', '限制或完全禁止内联脚本（inline script）及 eval 函数的执行', '在检测到违规资源加载时，将拦截日志发送到指定的违规上报服务', '在服务端对数据库索引文件的读取权限进行细粒度控制'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果一个网站的登录凭证没有存在 Cookie 而是存在 localStorage 中，黑客钓鱼网站还能实施 CSRF 攻击吗？不能。因为 localStorage 在跨站环境（B 网站）下具有同源策略（SOP）的物理隔离，B 网站发送请求时无法获取 A 网站的 localStorage，且浏览器不会像 Cookie 那样自动发送它，但要注意此模式依然极易受到 XSS 窃取。`),
        multiple('为了有效防御跨站请求伪造（CSRF）漏洞，以下哪些措施属于业界推荐的防护手段？', ['为 Session Cookie 配置 SameSite=Strict 或 Lax 属性以限制跨站携带', '在请求体或自定义 Header 中附带随机生成的 CSRF Token 进行双重校验', '校验请求头中的 Origin 和 Referer 属性，拒绝非白名单的第三方请求源', '在后端接口设计中，强制使用 GET 请求方法来承载所有涉及删除或修改的操作'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果应用配置了 HSTS（HTTP Strict Transport Security），为什么能进一步加固 Secure 属性？HSTS 会指示浏览器在本地强制把所有指向该网站的 HTTP 链接重定向为 HTTPS 链接，直接从客户端源头消除了任何发起明文 HTTP 请求的可能性，防止在首次重定向中发生凭证泄漏。`),
        multiple('在配置 HTTP 响应头中的 Set-Cookie 时，以下哪些 Cookie 属性直接影响其在浏览器中的发送范围与安全性？', ['设置 Domain 属性以决定 Cookie 能够向哪些子域名或主域名发送', '设置 Path 属性以限制 Cookie 仅在访问特定子路径路由时才被携带', '设置 SameSite 属性以防范在跨站（Cross-site）请求下凭证的自动附带', '设置全局的 font-weight 属性以控制浏览器在解析会话凭证时的字体加粗'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果 CDN 静态资源在部署时由于开启了 Gzip/Brotli 动态压缩，导致浏览器下载的内容哈希与前端 integrity 不一致，该怎么排查？SRI 校验针对的是解压后的“原始数据纯文本”，与传输层的压缩编码无关。若出现不一致，应检查是否在发布过程中第三方 CDN 修改了 JS 内容本身（如动态注入打点脚本或时间戳），此时应立刻终止加载。`),
        multiple('为了保障持续集成与交付（CI/CD）流水线的凭证与密钥安全，CI 凭证治理规范应包括：', ['遵循最小特权原则，仅赋予流水线账号执行构建发布所需的最小权限', '为敏感密钥配置短暂的有效期，或接入自动化的短期可轮换凭证服务', '对流水线控制台打印的所有运行输出日志执行敏感字符与密匙的自动脱敏', '为了简化操作，将具有永久管理员权限的主账号密钥直接托管在 public 脚本中'], [0, 1, 2], `💡 它解决了什么问题：
