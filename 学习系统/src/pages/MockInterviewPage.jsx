...
技术栈主要是 Vue 2、Vue 3 、vite，最近也在关注 AI Agent 的流式渲染、思考链的展示渲染。现在希望找一个业务和工程复杂度更高的平台，把复杂系统治理、性能优化和 AI 辅助研发这些经验继续用起来`
  ...
  const loadReverseQaTemplate = () => {
    setReverseQaText(
`1. 咱们团队目前核心的业务方向和前端技术栈是什么？现阶段最大的工程挑战是什么——是首屏性能、多人协作的状态一致性，还是跨团队的组件复用和维护成本？
2. 团队在做技术选型或架构决策时，通常是走 RFC/ADR 文档驱动的模式，还是以 PRD + 技术负责人拍板为主？Code Review 的粒度通常在哪个层面把关？
3. 如果我有幸能入职，试用期内的核心交付目标和最需要解决的痛点是什么？
4. 前端这边目前的线上质量保障是怎么做的——有没有完善的错误监控、性能基线和告警机制？这块主要是前端负责埋点上报，还是有专门的可观测性基础设施团队？
5. 我比较看重技术上的持续成长，想了解一下团队通常通过什么方式沉淀工程经验——比如内部分享、技术文档、还是会参与开源或行业交流？`
    );
  };
