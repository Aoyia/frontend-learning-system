...
### Q1 [single]
Vue 3 diff 性能提升的关键之一是编译阶段引入了什么标记机制？
A. VNodeHash
B. PatchFlag
C. TreeShakingFlag
D. HydrateMask
...
### Q3 [judgment]
在更新阶段，Block Tree 的目标是让 diff 复杂度更接近"动态节点数量"而非"整棵模板大小"。
答案：对
解析：正确。dynamicChildren 是只包含动态节点的扁平入口。

### Q4 [single]
在 `patchKeyedChildren` 中，先做"前置扫描 + 后置扫描"的主要目的是什么？
A. 为了生成 source map
...
### Q5 [single]
`newIndexToOldIndexMap` 中值为 `0` 通常表示什么？
A. 该节点在旧序列中存在且无需移动
B. 该节点是静态节点
C. 该新节点在旧序列中不存在，需要新建
D. 该节点需要立即卸载
...
### Q6 [multiple]
在中间乱序区使用 LIS（最长递增子序列）优化时，以下哪些说法正确？
...
### Q8 [judgment]
首次挂载阶段，静态节点也会被创建；只是在后续更新中通常会被优化跳过。
答案：对
解析：正确。首次 mount 需要真实创建 DOM；优化收益主要体现在后续 patch。

### Q9 [single]
在 block 优化路径中，`dynamicChildren` 更接近哪种角色？
A. 完整节点树的镜像副本
B. 只包含动态节点的扁平"快速通道"
...
### Q11 [judgment]
没有 key 的列表在复杂重排场景下，通常比有 key 的列表更容易做稳定复用。
答案：错
解析：错误。缺少 key 会降低匹配稳定性，重排场景更容易出现不必要 patch。

### Q12 [single]
为什么 v-for 相关 fragment 常被单独视为一个 block 边界？
A. 因为它只包含静态节点
...
### Q13 [single]
在中段 diff 中，如果某旧节点在 `keyToNewIndexMap` 中找不到对应新索引，应该如何处理？
A. 移动到末尾
B. 卸载该旧节点
C. 强制复用到最近节点
D. 标记为静态并跳过
...
### Q16 [single]
当用户手写 render 且缺乏编译器注入优化标记时，更常见的结果是？
