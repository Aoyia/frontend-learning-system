...
export function Home({ progressCache, petState, onOpenPet, onOpenRoadmap, onOpenBreaker, onOpenModule, onOpenMockInterview }) {
  return (
    <div data-component="home-page" className="max-w-[900px] mx-auto">
      <div data-element="hero" className="text-center py-10 px-0">
        <h1 className="text-[32px] font-bold mb-3 text-text-strong">🚀 中高级前端学习与面试校准 system</h1>
        <p className="text-text-secondary text-[15px]">先用技术破冰建立知识地图，再进入专题学习、作业、自测和错题复盘。</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 mt-1 mx-0 mb-7.5">
        <button
          data-element="entry-card"
        ...
        >
          <span className="text-[26px]">🎙️</span>
          <span className="text-[17px] font-bold text-text-strong">智能模拟面试</span>
          <span className="text-[13px] text-text-secondary leading-relaxed">全真面试流程、口试题库列表、智能评估与掌握跳过。</span>
        </button>
      </div>
      <PetHomeCard petState={petState} onOpen={onOpenPet} />
      <div data-element="module-grid" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-8">
          ...
          return (
            ...
            >
              ...
              <div className="mt-3.5 flex items-center gap-2.5">
                <div data-element="progress-bar" className="flex-1 h-1 bg-border rounded-[2px] overflow-hidden">
