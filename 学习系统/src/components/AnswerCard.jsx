...
export default function AnswerCard({ quizState, collapsed, mobileOpen, immersiveMode, onToggle, onJumpToQuestion }) {
  ...
  const sidebarClass = `flex flex-col h-full bg-surface border-l border-border overflow-hidden transition-all duration-200 ` +
    ...
    (immersiveMode ? 'md:hidden ' : '') + // 沉浸模式下，桌面端隐藏原有面板，改由 dots-nav 渲染
  ...
  return (
    <>
      {/* 沉浸模式下的桌面端极简呼吸点侧边栏（滑出毛玻璃面板） */}
      {immersiveMode && (
        <div className="immersive-dots-nav-container hidden md:flex">
          {/* 常驻极窄呼吸点轨道 */}
          <div className="immersive-dots-track">
            {groups.flatMap(g => g.cells).map(cell => {
              const isCurrent = cell.gi === quizState.currentPageIdx;
              return (
                <button
                  key={cell.gi}
                  className={`immersive-dot ${cell.cls} ${isCurrent ? 'active' : ''}`}
          ...
          {/* 鼠标移入时滑出的毛玻璃详细控制面板 */}
          <div className="immersive-hover-card-panel">
            <div className="flex items-center justify-between p-3.5 border-b border-border dark:border-white/10 font-bold text-[13px] text-text-strong dark:text-white">
              <span>答题进度</span>
              <span className="text-text-secondary dark:text-white/60 text-[11px] font-medium">已答 {correctCnt + wrongCnt + selectedCnt} / {totalQ}</span>
            </div>
            <div className="flex-1 overflow-y-auto p-3 px-3.5">
              {groups.map(group => (
                <div className="mt-3.5 first:mt-0" key={group.pageNo}>
                  <div className={`text-[10px] text-text-secondary/60 dark:text-white/50 uppercase tracking-widest mb-1.5 font-bold ${group.isCurrent ? 'text-primary' : ''}`}>
                    P{group.pageNo}{group.isCurrent ? ' 当前' : ''}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {group.cells.map(cell => {
                      let cellCls = "";
                      if (cell.cls === 'correct') cellCls = "bg-success text-white border-success";
                      else if (cell.cls === 'wrong') cellCls = "bg-danger text-white border-danger";
                      else if (cell.cls === 'selected-unsub') cellCls = "bg-primary text-white border-primary";
                      else cellCls = "bg-surface-alt dark:bg-white/5 text-text-secondary dark:text-white/60 border-border dark:border-white/10";

                      return (
                        <div
                          key={cell.gi}
                          className={`w-7.5 h-7.5 rounded-[7px] flex items-center justify-center text-[12px] font-bold cursor-pointer border transition-all duration-150 select-none hover:brightness-125 ${cellCls} ${group.isCurrent ? 'ring-2 ring-primary ring-offset-1 ring-offset-black' : ''}`}
                          title={`第 ${cell.gi + 1} 题 · ${cell.title}`}
                          onClick={() => onJumpToQuestion(cell.gi)}
                        >
                          {cell.gi + 1}
            ...
            <div className="p-3 border-t border-border dark:border-white/10 grid grid-cols-2 gap-2 text-[11px] text-text-secondary dark:text-white/60">
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-success" />对 {correctCnt}</div>
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-danger" />错 {wrongCnt}</div>
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-primary" />已选 {selectedCnt}</div>
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-border dark:bg-white/20" />未做 {unansweredCnt}</div>
            </div>
          </div>
        </div>
      )}

      {/* 原本的答题卡（普通模式，或者沉浸模式下的移动端） */}
