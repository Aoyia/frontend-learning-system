...
真正项目里经常是混合架构，而不是全站一种方案。`,
      ...
      quiz: [
        single('在为前端项目评估和选择渲染方案（如 CSR、SSR、SSG、ISR）时，首先应当考量的依据是：', ['具体业务场景的用户体验核心指标（SEO、首屏加载要求）以及页面内容更新频率。', '当前社区中哪个框架的技术名词或底层概念更新颖。', '当前项目组内资深架构师更偏爱哪一套技术栈的生态系统。', '项目在构建发布后，输出的静态 HTML 文件名字符长度限制。'], 0, `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：对于一个高度个性化（如根据用户地理位置推荐不同商品）、又强依赖 SEO 的电商首页，在选型上如何取舍？应采用“动态SSR + CDN 边缘缓存（Edge Rendering）”或“SSG + 客户端二次 Hydration 动态填充”。将通用商品骨架静态生成，个性化数据在客户端异步拉取，以此兼顾 SEO 与首屏速度。`),
        multiple('在前瞻性架构设计中选择服务端渲染（SSR）方案时，团队通常需要付出的工程代价包括？', ['需要提供并维护更高并发计算负载的服务端 Node.js 容器容量', '需要在浏览器端执行 Hydration 并注入全量的运行时 JS bundle', '需要精心设计服务端接口缓存、HTML 缓存与页面降级防雪崩策略', '不需要在客户端执行任何 JavaScript 逻辑代码即可完成全页渲染'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如何在大流量 SSR 页面中，在不影响 LCP 的前提下优化 TTFB？应该使用“流式渲染（Streaming HTML Rendering）”。服务端不需要等所有数据接口都返回，而是立即先发送静态骨架的 HTML 头部，并在后续数据返回时，以分块（Chunk）形式源源不断地向浏览器流式推送剩余 HTML，从而将 TTFB 压缩到极限。`),
        single('对于一个强登录态限制、无任何 SEO 推广需求的企业内部 ERP 管理后台，最适宜的渲染技术方案是：', ['客户端单页渲染（CSR）配合精细化的按需加载与 HTTP 缓存优化。', '在构建打包阶段，对应用进行全站级别的静态站点生成（SSG）打包部署。', '服务端不提供任何 JS 逻辑，仅在用户访问时返回不具备交互能力的纯物理 HTML 骨架。', '通过取消前端路由跳转，让用户每次点击均触发整个浏览器页面的硬重载。'], 0, `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：在 CSR 架构下，如何解决首次加载时主 JS Bundle 过大导致的白屏时间长的问题？必须实施严格的“代码分割（Code Splitting）”和“路由懒加载”，并配合在 index.html 中硬编码骨架屏（Skeleton Screen）或使用 Service Worker 提前预拉取关键资产，确保白屏体验不弱于 SSR。`),
        multiple('静态站点生成（SSG）方案，最适合应用于以下哪些高 SEO 要求且低动态更新的场景？', ['产品设计规范、技术使用文档或帮助中心页面', '新闻资讯、技术博客或更新日志（Changelog）发布页面', '用于广告投放、产品展示的活动页和公共营销页', '需要强登录鉴权且每个用户展示数据完全不同的实时控制台页面'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果是拥有 10 万个商品详情页的电商系统，采用纯 SSG 会导致构建生成时间长达数小时，且无法处理库存 and 价格的实时更新。如何优化？应采用'ISR（增量静态生成）'或'On-demand Revalidation'。构建时只生成最热门的 1000 个商品页，其余页在用户首次访问时按需在后台生成并缓存，并配置失效时间窗口或通过 Webhook 精准使页面失效重建。`),
        single('应用采用服务端渲染（SSR）部署上线后，监控显示 TTFB（首字节时间）居高不下，应优先排查：', ['服务端数据请求耗时、服务端缓存机制缺失以及服务端组件的渲染执行成本。', '前端组件样式表中，个别全局按钮的覆盖规则与边框颜色冲突。', '打包生成的 CSS 选择器命名嵌套过深导致的浏览器排版树重计算时延。', '代码提交历史中，因 Git 提交邮箱前缀与持续集成环境安全策略不符。'], 0, `💡 它解决了什么问题：
...
服务端和客户端时间不同，可能导致 mismatch。应把不稳定值放到客户端 effect，或从服务端稳定传入同一份数据。`,
      ...
      quiz: [
        single('在同构渲染（SSR）体系中，术语 Hydration（注水/激活）的核心工作职责是：', ['让客户端的 JavaScript 代码接管服务端返回的静态 HTML 并绑定事件与交互状态。', '通过将资源映射到本地缓存，以缩减首屏大图片在网络传输中的大小。', '向本地域名解析服务器发起预建连，以加快 CDN 资源的静态下载效率。', '在服务端运行时自动建立与微服务集群后台数据库的持久物理连接。'], 0, `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果在此空窗期用户频繁点击输入框却无反应，如何进行用户体验降级兜底？可利用组件级懒加载和 Selective Hydration 异步分水激活交互区，或使用 CSS 的 :active 样式和 inline JS 原生劫持方案收集前置点击事件，等 Hydrate 完成后再分发回放。`),
        single('同构框架中，服务端渲染引入流式 HTML 渲染（Streaming SSR）的最核心性能优势是：', ['允许服务端以数据分片形式更早地向浏览器发送 HTML 块，降低用户感知的第一时间开销。', '使得应用程序能够完全在开发机本地脱离任何真实的 Node.js 服务端运行环境。', '在构建打包时通过流式压缩机制，直接废除项目中所有的 CSS 样式文件。', '拦截所有的异步 bundle 资源加载，以此防止前端执行任何 JavaScript 代码。'], 0, `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果在 Streaming 渲染过程中，Node 服务端在发送后半部分 HTML 时发生了未捕获的运行时异常（比如接口突然报错），前端在页面上表现为什么？怎么容灾？如果发送头部后出错，HTTP 状态码（200）已无法更改。React 客户端会在浏览器收到残残 HTML 时自动抛出错误，并在客户端降级为 CSR 重新拉取该部分数据，保障了整体界面的完整性。`),
        multiple('为了优化同构 SSR 应用在首屏加载时的注水（Hydration）开销，可以考虑的优化方向包括？', ['尽量减小客户端组件（Client Component）的作用域范围', '对客户端 JS 进行代码分割（Code Splitting）以减少首屏加载大小', '使用 defer/async 或在 idle 状态下延后执行非关键交互脚本', '在构建打包时将大量不相关的第三方依赖强行合并进公共 entry chunk 中'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如何在打包阶段审计出哪些第三方依赖库被无意间带入了客户端的 Hydration 队列中？可以使用 Webpack Bundle Analyzer 或 Vite Bundle Visualizer 插件，配合在 React 源码中查找带有 "use client" 指令的边界。如果在顶层组件错误声明了 use client，会导致其下的所有依赖包（即使只是格式化日期的 moment.js）被全量打入客户端 JS 中，必须下沉 use client 到精确的交互叶子节点组件中。`),
        single('用户反馈使用 SSR 访问页面时，首屏渲染速度极快（FP/FCP 很好），但有那么几秒钟，点击任何按钮均无响应，这常代表：', ['服务端的 Hydration 流程或核心 bundle 脚本尚未在客户端加载并执行完成。', '首屏内容中的某张背景图片缺少 alt 标签导致浏览器产生了视觉假死。', '某个数据接口返回了 HTTP 204（No Content）导致全局交互链路被阻断。', '项目根目录下的 README.md 说明中，未能对业务依赖的包版本做详细陈述。'], 0, `💡 它解决了什么问题：
...
服务端取数，客户端负责交互。不要为了方便把整棵树都标成客户端组件。`,
      ...
      quiz: [
        single('在 React 架构设计中，React Server Component (RSC) 的最核心技术特征是：', ['仅在服务端运行，且其代码和依赖库绝不会被打包进客户端 the JS Bundle 中。', '必须且只能在浏览器的沙箱环境内，利用 window 全局 API 进行动态注册执行。', '为了管理页面交互状态，必须在组件内部引入并注册 useState 状态钩子。', '安全沙箱机制完全限制了其直接访问服务端的本地文件系统或数据库资源的权限。'], 0, `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如何在保持叶子节点为 Client Component 的情况下，让 Server Component 依然能嵌套在它的内部运行？可以利用“插槽/Children 传递机制”。将 Server Component 作为 props.children 传入 Client Component 中。由于 children 的解析在服务端已经完成并序列化，客户端组件仅作为容器渲染它，而不会将 children 对应的组件代码打包进客户端，实现了完美的嵌套设计。`),
        single('在使用 RSC 时，当需要将数据从服务端组件（Server Component）传递给客户端组件（Client Component）时，其数据类型要求必须是：', ['支持在网络传输中被解析和重构的可序列化（Serializable）数据。', '必须是一个可在客户端直接触发和执行的回调函数（Callback Function）。', '包含有敏感数据库连接凭证与读写权限的操作对象实例。', '必须是已经由服务端渲染生成并带有样式定义的物理 DOM 节点对象。'], 0, `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：如果服务端组件确实需要将一个异步的数据获取 Promise 传给客户端组件，这能被序列化吗？如何实现？React 18 特殊支持了 Promise 的序列化传递。服务端可以将一个未 resolve 的 Promise 作为 props 传给客户端组件，客户端组件在内部使用最新的 use hook 接收并结合 Suspense 挂起等待决议，这为跨边界异步协作提供了原生通道。`),
        multiple('在 React 体系中，引入 React Server Components (RSC) 底层设计的工程红利包括？', ['将纯静态的渲染逻辑留在服务端，有效缩减客户端的 JS Bundle 物理体积', '拉近渲染端与数据源的物理距离，在服务端直接执行高性能数据库取数', '让敏感的接口密钥和私有业务库完全留在 Node 服务端以防范泄露', '能够自动优化和解决客户端交互在点击事件响应时的一切 INP 退化问题'], [0, 1, 2], `💡 它解决了什么问题：
...
3. 进一步拓展大厂面试追问：在 RSC 下，如果服务端组件的数据源频繁变化（如实时报价），为了保持更新而频繁重算 RSC，是否会对 Node 服务器造成极大的 CPU 算力损耗？是的。因此，必须对 Server Component 的数据源调用加上合理的缓存策略（如 Next.js 的 fetch cache 或者是 unstable_cache），对静态和半静态数据进行缓存隔离，防止并发流量直接拖挂 Node 服务。`),
        single('关于 React Server Component (RSC) 与传统服务端渲染（SSR）的关系，以下表述最准确的是：', ['两者是相辅相成但完全不同的技术维度：RSC 决定组件如何生成，SSR 决定数据如何向 HTML 渲染分发。', '两者的底层工作机制完全重叠，仅属于不同前端框架在命名术语上的叫法差异。', '两者属于完全互斥的技术选型，在同一个前端工程中严禁将两者混用或同时开启。', '它们都是为 CSS 样式而生的工程化处理方案，旨在提升样式解析与样式覆盖的速度。'], 0, `💡 它解决了什么问题：
