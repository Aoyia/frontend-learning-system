...
function renderQuizPage() {
  ...
  renderAnswerCard();
  // 进入测验页时显示答题卡侧栏
  const acSidebar = document.getElementById('ac-sidebar');
  if (acSidebar) acSidebar.style.display = 'flex';
}
...
function showQuizResult() {
  ...
  // 隐藏答题卡侧栏
  const acSidebar = document.getElementById('ac-sidebar');
  if (acSidebar) acSidebar.style.display = 'none';
...
function toggleAnswerCard() {
  ...
  // 更新折叠按钮圖标方向
  const btn = sidebar.querySelector('.ac-toggle-btn');
...
function renderAnswerCard() {
  ...
  let correctCnt = 0, wrongCnt = 0, selectedCnt = 0, unansweredCnt = 0;
  let html = '';

  for (let p = 0; p < totalPages; p++) {
    const pageStart = p * s.pageSize;
    const pageEnd   = Math.min(pageStart + s.pageSize, totalQ);
    const isSubmittedPage = s.submittedPages.has(p);
    const isCurPage = p === s.currentPageIdx;
    html += `<div class="answer-card-page-label">第 ${p + 1} 页${isCurPage ? ' （当前）' : ''}</div>`;
    html += `<div class="answer-card-grid">`;
    for (let gi = pageStart; gi < pageEnd; gi++) {
      const q   = s.questions[gi];
      let cls, title;
      if (isSubmittedPage) {
        const ok = isAnswerCorrect(q, s.answers[gi]);
        cls = ok ? 'correct' : 'wrong';
        title = ok ? '答对' : '答错';
        if (ok) correctCnt++; else wrongCnt++;
      } else if (s.selections[gi] !== undefined) {
        cls = 'selected-unsub';
        title = '已选未提交';
        selectedCnt++;
      } else {
      ...
      const curPageCls = isCurPage ? ' is-current-page' : '';
      html += `<div class="ac-cell ${cls}${curPageCls}" title="第 ${gi+1} 题 · ${title}" onclick="jumpToQuestion(${gi})">${gi + 1}</div>`;
    }
    html += `</div>`;
  }
  ...
  body.innerHTML = html;
  summary.innerHTML = [
    `<div class="ac-sum-item"><div class="ac-sum-dot" style="background:var(--success)"></div>答对 ${correctCnt}</div>`,
    `<div class="ac-sum-item"><div class="ac-sum-dot" style="background:var(--danger)"></div>答错 ${wrongCnt}</div>`,
    `<div class="ac-sum-item"><div class="ac-sum-dot" style="background:var(--accent)"></div>已选未交 ${selectedCnt}</div>`,
    `<div class="ac-sum-item"><div class="ac-sum-dot" style="background:var(--border)"></div>未做 ${unansweredCnt}</div>`,
  ].join('');
}

function jumpToQuestion(globalIdx) {
  const s = quizState;
  const targetPage = Math.floor(globalIdx / s.pageSize);
  if (targetPage !== s.currentPageIdx) {
    s.currentPageIdx = targetPage;
    renderQuizPage(); // 会重新设置侧栏显示
  }
  // 滚动到对应题目
  setTimeout(() => {
