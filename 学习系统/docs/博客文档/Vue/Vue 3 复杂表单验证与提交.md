...
### Q2 [multiple]
一个可维护的表单 composable 通常应该暴露哪些能力？
A. formState
B. errors
...
### Q7 [judgment]
Vue 官方 composable 推荐返回多个 refs，方便组件解构后仍保持响应式连接。
答案：对
解析：这是 composable 设计的重要约定，避免解构普通 reactive 对象后丢失响应式连接。

### Q8 [multiple]
排查“用户明明填了表单但提交失败”时，应该检查哪些路径？
A. 前端同步规则是否误判
B. 异步校验是否覆盖了最新输入
C. 接口错误是否正确映射到字段
D. 是否完全依赖 alert 展示错误
