...
### Q1 [single]
在 Vue 3 defineAsyncComponent 异步组件场景中，`defineAsyncComponent(() => import('./Foo.vue'))` 最核心的收益是什么？
A. 让组件变成全局组件
...
### Q2 [single]
在 Vue 3 defineAsyncComponent 异步组件场景中，异步组件的真实 JS chunk 主要由谁生成？
A. Vue 的运行时
B. 浏览器 DOM API
...
### Q3 [judgment]
在 Vue 3 defineAsyncComponent 异步组件场景中，调用 `defineAsyncComponent` 时，真实组件文件一定会立刻被下载。
答案：错
解析：它只创建异步组件包装器，通常在组件第一次渲染时才执行 loader 并触发下载。

### Q4 [multiple]
在 Vue 3 defineAsyncComponent 异步组件场景中，`defineAsyncComponent` 的高级配置通常可以处理哪些状态？
A. loadingComponent
B. errorComponent
C. delay
D. timeout
...
### Q5 [single]
在 Vue 3 defineAsyncComponent 异步组件场景中，如果网络很快，为什么 loading 默认有 delay？
...
### Q6 [multiple]
在 Vue 3 defineAsyncComponent 异步组件场景中，排查异步组件加载失败时，应该优先检查哪些点？
...
### Q7 [judgment]
在 Vue 3 defineAsyncComponent 异步组件场景中，异步组件包装器会把 props 和 slots 透传给内部真实组件。
答案：对
解析：官方文档说明异步包装组件可以无缝替换原组件使用，会向内部组件传递 props 和 slots。

### Q8 [single]
在 Vue 3 defineAsyncComponent 异步组件场景中，在 SSR 场景下，Vue 3.5+ 异步组件新增的 lazy hydration 策略主要解决什么问题？
A. 控制组件什么时候在客户端水合
B. 删除所有服务端 HTML
C. 禁止动态 import
D. 强制所有组件同步加载
