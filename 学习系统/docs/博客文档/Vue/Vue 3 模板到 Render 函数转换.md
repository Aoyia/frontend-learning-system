...
### Q1 [single]
在 Vue 3 模板编译到 render 函数的场景中，Vue3 模板编译的三个核心阶段顺序是？
...
### Q2 [multiple]
在 Vue 3 模板编译到 render 函数的场景中，以下关于 Parse 阶段的描述，哪些正确？
...
### Q3 [single]
在 Vue 3 模板编译到 render 函数的场景中，Transform 阶段的核心产物是什么？
A. 最终的 render 函数字符串
...
### Q4 [judgment]
在 Vue 3 模板编译到 render 函数的场景中，`app.mount()` 时，如果组件没有 render 函数和 template 选项，Vue 会报错。
答案：错
解析：Vue 会用 `container.innerHTML` 作为模板，通过运行时编译器将其编译为 render 函数，然后再挂载。

### Q5 [multiple]
在 Vue 3 模板编译到 render 函数的场景中，静态提升（Static Hoisting）优化的核心是什么？
...
### Q6 [single]
在 Vue 3 模板编译到 render 函数的场景中，PatchFlag 值为 `1` 代表什么？
A. 动态 class
B. 动态文本内容（TEXT）
C. 动态样式
D. 需要完整 diff
...
### Q7 [multiple]
在 Vue 3 模板编译到 render 函数的场景中，Block Tree 的优化作用包括哪些？
...
### Q8 [single]
在 Vue 3 模板编译到 render 函数的场景中，`setupRenderEffect` 函数的核心职责是什么？
A. 编译模板为 AST
...
### Q9 [judgment]
在 Vue 3 模板编译到 render 函数的场景中，Vue3 在生产构建时（通过 Vite/Webpack），模板编译发生在浏览器运行时。
答案：错
解析：推荐（也是默认）模式是构建时编译（编译器只在构建工具中运行），打包产物只包含运行时（runtime-core），不包含编译器，体积更小，启动更快。

### Q10 [single]
在 Vue 3 模板编译到 render 函数的场景中，`compileToFunction` 中 `new Function('Vue', code)(runtimeDom)` 这行代码的作用是？
...
### Q11 [multiple]
在 Vue 3 模板编译到 render 函数的场景中，以下哪些是 Transform 阶段处理的内容？
...
### Q12 [single]
在 Vue 3 模板编译到 render 函数的场景中，以下哪个调用链描述了 Vue3 运行时编译的正确路径？
...
### Q13 [judgment]
在 Vue 3 模板编译到 render 函数的场景中，Transform 阶段的 NodeTransform 和 DirectiveTransform 是同一类转换器，可以互换使用。
答案：错
解析：两者职责不同。NodeTransform 处理元素、文本等节点类型；DirectiveTransform 专门处理 v-model、v-on、v-bind 等指令，返回该指令对应的 props 信息。

### Q14 [multiple]
在 Vue 3 模板编译到 render 函数的场景中，关于 createApp → mount 的整体流程，以下哪些正确？
...
### Q15 [single]
在 Vue 3 模板编译到 render 函数的场景中，Vue3 编译优化中，哪个机制让 diff 从"全树比较"变为"靶向更新"？
A. 静态提升
...
### Q16 [judgment]
在 Vue 3 模板编译到 render 函数的场景中，`_createElementBlock` 和 `_createElementVNode` 创建的 VNode 是同一种类型，没有区别。
答案：错
