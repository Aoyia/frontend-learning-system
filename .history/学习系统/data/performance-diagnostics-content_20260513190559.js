...
这个 demo 的重点不是上报 API，而是每条指标必须带页面、网络、版本等维度，否则无法定位具体人群。`,
      ...
      references: ['web.dev Core Web Vitals 文档', 'web-vitals npm 包文档', 'Chrome DevTools Performance 文档', 'MDN Performance API'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'LCP element', desc: '被浏览器认定为最大内容绘制的元素，可能是图片、文本块或视频首帧。' },
        { term: 'TTFB', desc: 'HTML 第一个字节到达前的时间。' },
        { term: 'preload', desc: '提示浏览器提前发现和加载关键资源。' },
        { term: 'fetchpriority', desc: '给浏览器资源优先级提示，常用于首屏关键图。' },
        { term: 'critical CSS', desc: '首屏渲染必须的 CSS。' },
        { term: 'render delay', desc: '资源已到但元素仍因 JS、CSS 或渲染阻塞而没有绘制。' },
      ],
...
不要对所有图片都 preload。只给真正的首屏 LCP 候选资源提高优先级，否则会抢占带宽。`,
      ...
      references: ['web.dev Optimize LCP', 'MDN rel=preload', 'MDN fetchpriority', 'Chrome DevTools Performance 文档'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'input delay', desc: '用户输入发生后，事件回调开始执行前的等待。' },
        { term: 'processing duration', desc: '事件回调执行和相关同步逻辑耗时。' },
        { term: 'presentation delay', desc: '回调后到下一次绘制之间的延迟。' },
        { term: 'Long Task', desc: '主线程上超过 50ms 的任务。' },
        { term: 'yield', desc: '让出主线程，让浏览器有机会处理输入和绘制。' },
        { term: 'virtualization', desc: '只渲染视口附近内容，减少 DOM 和渲染成本。' },
      ],
...
真实项目里还可以根据场景使用 Web Worker、scheduler.yield、requestIdleCallback 或服务端分页。关键不是 API 名字，而是避免长时间独占主线程。`,
      ...
      references: ['web.dev INP 文档', 'web.dev Optimize INP', 'Chrome Long Tasks API', 'MDN Web Workers'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'CLS', desc: '累计布局偏移，衡量非预期视觉移动。' },
        { term: 'layout shift', desc: '可见元素起始位置发生变化。' },
        { term: 'aspect-ratio', desc: '提前声明宽高比例，避免媒体资源加载后撑开布局。' },
        { term: 'font-display', desc: '控制字体加载期间文本显示策略。' },
        { term: 'placeholder', desc: '为异步内容保留接近最终尺寸的空间。' },
        { term: 'hydration mismatch', desc: '服务端 HTML 与客户端渲染结果不一致，可能引发布局变化。' },
      ],
...
CLS 优化不是不让内容变化，而是让变化发生在预期空间里。`,
      ...
      references: ['web.dev CLS 文档', 'web.dev Optimize CLS', 'Chrome DevTools Layout Shifts 文档', 'MDN aspect-ratio'],
      quiz: [
    ...
    makeLongformDoc({
      ...
      keywords: [
        { term: 'performance budget', desc: '对体积、指标和资源数量设置可执行阈值。' },
        { term: 'bundle analysis', desc: '分析构建产物体积、重复依赖和 chunk 组成。' },
        { term: 'release comparison', desc: '按版本对比性能和错误指标，判断退化是否由发布引入。' },
        { term: 'JS Heap', desc: 'JavaScript 堆内存，用于观察对象占用趋势。' },
        { term: 'Detached DOM', desc: '已脱离页面但仍被 JS 引用的 DOM，常见泄漏源。' },
        { term: 'Retainers', desc: 'Memory 快照中显示对象为什么仍可达的引用链。' },
      ],
