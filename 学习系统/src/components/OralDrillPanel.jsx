...
`;
  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-slide-up pb-8">
      {/* 题头真题背景 */}
      <div className="bg-surface-alt/45 p-5 rounded-xl border border-border/40 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        {/* 左栏：模拟作答区 */}
        <div className="bg-surface border border-border/30 rounded-xl p-5 shadow-sm flex flex-col gap-5">
          <h3 className="text-sm font-bold text-text-secondary border-b border-border/30 pb-2.5 m-0 flex items-center justify-between">
          ...
          {status === 'answering' ? (
            <div className="flex flex-col gap-5 py-2">
              {/* 计时器条 */}
              <div className="flex items-center justify-between gap-4 bg-surface-alt/30 p-4 rounded-xl border border-border/20">
                <div className="flex items-center gap-3.5">
                  <div className="w-12 h-12 rounded-full border-2 border-primary text-primary flex items-center justify-center font-mono font-bold text-sm">
                    {formatTime(seconds)}
                  </div>
                  <div>
              ...
              <div className="border border-border/30 rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full bg-surface-alt/30 hover:bg-surface-alt/50 border-0 px-4 py-2.5 text-xs text-text-secondary font-semibold text-left cursor-pointer transition-colors duration-150 flex items-center justify-between"
                >
                  <span>💡 查看作答思路线索 (脑子一片空白时点此)</span>
                  <span>{showTips ? '▲ 折叠' : '▼ 展开'}</span>
                </button>
                {showTips && (
                  <div className="p-4 bg-surface text-xs text-text-secondary/80 leading-relaxed border-t border-border/20">
                    {q.recommendStructure ? (
                      <p>“您可以从结论开始说起，重点覆盖下面提到的一些技术关键词。例如这道题关键要说清如何给静态和动态做隔离优化...”</p>
                    ) : (
              ...
              >
                我记录好了，查看参考答案进行 AI 智能评估
              </button>
            </div>
          ) : (
            // 已经是自评或者已提交状态：展示之前的记录
            <div className="flex flex-col gap-4 py-2">
              <div className="bg-surface-alt/25 p-4 rounded-xl border border-border/20 flex flex-col gap-3">
              ...
              <div className="text-xs text-text-secondary/70 bg-surface-alt/20 p-3 rounded-lg border border-border/20">
        ...
        {/* 右栏：参考自评区 */}
        <div className={`bg-surface border rounded-xl p-5 shadow-sm flex flex-col gap-5 ${
          status !== 'answering' ? 'border-primary/40' : 'border-border/30'
        }`}>
          <h3 className="text-sm font-bold text-text-secondary border-b border-border/30 pb-2.5 m-0 flex items-center justify-between">
          ...
          {status === 'answering' ? (
            <div className="flex flex-col items-center justify-center py-16 text-center select-none opacity-30">
              <svg className="w-10 h-10 text-text-secondary/60 mb-3" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
                <path d="M7 11V7a5 5 0 0 1 10 0v4" />
              </svg>
              <div className="text-xs font-bold text-text-secondary">参考答案与评估项已被隐藏</div>
              <div className="text-[11px] text-text-secondary/70 mt-1">输入完毕点击左下角按钮即可解封答案与关键词列表</div>
            </div>
          ) : aiLoading ? (
            // 脑电波式科技感 AI 加载状态
      ...
      {q.followupQuestion && (status === 'evaluating' || status === 'submitted') && (
        <div className="mt-2 border border-primary/20 bg-surface rounded-2xl overflow-hidden shadow-md animate-slide-up select-none">
          <div className="bg-primary/5 px-5 py-3.5 border-b border-primary/10 flex justify-between items-center">
            ...
            {!showFollowup && (
              <button
                ...
                className="px-4 py-2 rounded bg-primary hover:bg-primary-hover text-white text-[11px] font-bold cursor-pointer transition-all border-0 shadow-sm active:scale-95"
              >
                接受追问挑战！
              </button>
            )}
          ...
          {showFollowup && (
            <div className="p-5 flex flex-col gap-4">
              {/* 追问题干 */}
              <div className="bg-surface-alt/45 p-4 rounded-xl border border-border/40 relative">
